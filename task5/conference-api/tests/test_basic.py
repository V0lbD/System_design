...
async def _login(service_client, login: str) -> str:
    response = await service_client.post(
        '/auth/login',
        headers={
            'X-RateLimit-Key': _unique(f'login_{login}'),
        },
        json={
            'login': login,
            'password': '123456',
...
async def test_talks_cache_is_invalidated_after_create_talk(service_client):
    login = _unique('cache_talks_user')
    await _register_user(service_client, login, 'speaker')
    token = await _login(service_client, login)

    headers = {'Authorization': f'Bearer {token}'}

    # First GET fills cache for /talks
    initial_response = await service_client.get('/talks')
    assert initial_response.status == 200, initial_response.text

    title = f'Cached Talks Invalidation {login}'
    ...
    create_response = await service_client.post(
        ...
        json={
            'title': title,
            'description': 'This talk must appear after cache invalidation',
        },
    )

    assert create_response.status == 201, create_response.text
    created_talk = create_response.json()

    # If cache was not invalidated, this GET could return stale data
    talks_response = await service_client.get('/talks')
    assert talks_response.status == 200, talks_response.text

    talks = talks_response.json()
    assert any(
        talk['id'] == created_talk['id'] and talk['title'] == title
        for talk in talks
    )


async def test_conference_talks_cache_is_invalidated_after_add_talk(service_client):
    login = _unique('cache_conference_user')
    await _register_user(service_client, login, 'organizer')
    token = await _login(service_client, login)
    ...
    talk_response = await service_client.post(
        ...
        headers=headers,
        json={
            'title': f'Conference Cache Talk {login}',
            'description': 'This talk must appear in conference after cache invalidation',
        },
    )
    assert talk_response.status == 201, talk_response.text
    talk = talk_response.json()
    ...
    conference_response = await service_client.post(
        ...
        headers=headers,
        json={
            'title': f'Conference Cache Test {login}',
            'description': 'Conference for cache invalidation test',
            'date': '2026-08-01',
        },
    )
    assert conference_response.status == 201, conference_response.text
    ...
    # First GET fills cache with empty conference talks list
    initial_talks_response = await service_client.get(
        f'/conferences/{conference_id}/talks',
    )
    assert initial_talks_response.status == 200, initial_talks_response.text
    assert initial_talks_response.json() == []
    ...
    # If cache was not invalidated, this GET could still return empty list
    conference_talks_response = await service_client.get(
        f'/conferences/{conference_id}/talks',
    )
    assert conference_talks_response.status == 200, conference_talks_response.text
...
async def test_login_rate_limit_returns_429_and_headers(service_client):
    rate_limit_key = _unique('rate_limit_login')
    ...
    for _ in range(5):
        response = await service_client.post(
            '/auth/login',
            headers={
                'X-RateLimit-Key': rate_limit_key,
            },
            json={
                'login': 'wrong_login',
    ...
    assert limited_response.status == 429, limited_response.text
