...
При запуске API из VS Code devcontainer контейнер devcontainer должен быть подключён
к Docker network, где запущен conference-postgres. В этом режиме используется
configs/config_vars.local.yaml с хостом conference-postgres.
